...
static void CodeGen_init(CodeGen *cg, const char *output_filename)
{
...
#define CodeGen_unreachable(cg) CodeGen_fail0(cg, __func__, "unreachable")
#define CodeGen_fail(cg, message, ...) CodeGen_fail0(cg, __func__, message)
static _Noreturn void CodeGen_fail0(CodeGen *cg, const char *function, const char *message)
{
    (void) cg;
    std_printf("lowering failed: %s: %s\n", function, message);
    std_exit(1);
}

#define CodeGen_expect(cg, condition) CodeGen_expect0(cg, condition, __func__, #condition)
#define CodeGen_expect2(cg, condition, message) CodeGen_expect0(cg, condition, __func__, message)
static void CodeGen_expect0(CodeGen *cg, bool condition, const char *function, const char *message)
{
    if (!condition) CodeGen_fail0(cg, function, message);
}
...
static Buffer Sema_evalTypeName0(CodeGen *cg, Node *n)
{
    switch (n->tag) {
        case node_type_expr:
            return Sema_evalTypeName0(cg, n->data.type_expr.type_expr);
...
static void CodeGen_genTypeExpr(CodeGen *cg, NodeDataTypeExpr expr)
{
    ...
    for (uint32_t i = 0; i < suffix_expr.suffixes_len; i++) {
        switch (suffix_expr.suffixes[i]->tag) {
...
static void CodeGen_genFunctionProto(CodeGen *cg, NodeDataFnProto fn_proto, bool is_static)
{
    if (is_static) emit("static ");
    ...
    if (!fn_proto.params) {
        ...
        NodeDataParamDeclList params = fn_proto.params->data.param_decl_list;
        CodeGen_fail(cg, "parameters in functions not supported");
        (void)params;
    }
}
...
static void CodeGen_genFunction(CodeGen *cg, NodeDataDeclFn fn, bool is_static)
{
    ...
    CodeGen_genFunctionProto(cg, fn.fn_proto->data.fn_proto, is_static);
    emit("\n{\n");
    cg->indent++;
...
static void CodeGen_genPrologue(CodeGen *cg)
{
    ...
    emitl("/* zig.h begin */");
    std_writeFile(zig_h, zig_h_len, 1, cg->out);
    emitl("/* zig.h end */");
    emitl("");
#else
    ...
    for (uint32_t i = 0; i < members.decls_len; i++) {
        if (members.decls[i]->tag == node_top_level_decl) {
            ...
            if (top_level_decl.decl->tag == node_decl_fn) {
                ...
                CodeGen_genFunctionProto(cg, fn_proto, !top_level_decl.is_pub);
                emitl(";");
            }
        }
    }
